...
Иначе воспользуйтесь данным скриптом, чтобы получить статическую библиотеку, которую затем сможете слинковать с Вашим проектом

``` bash
sh scripts/build_target.sh ваш_файл_для_анализа.cpp
```

На выходе получится статическая библиотека `libres.a`, которая будет находиться в папке `log`. После сборки Вашего проекта и запуска его, проследуйте следующим действиям
